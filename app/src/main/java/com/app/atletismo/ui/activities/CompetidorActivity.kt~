...
import com.app.atletismo.databinding.ActivityCompetidoresBinding
...
class CompetidoresActivity : AppCompatActivity() {

    private lateinit var binding: ActivityCompetidoresBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        enableEdgeToEdge()
        binding = ActivityCompetidoresBinding.inflate(layoutInflater)
            ...
            override fun onResponse(call: Call<List<Resultado>>, response: Response<List<Resultado>>) {
                if (response.isSuccessful && response.body() != null) {
                    ...
                    if(resultados.isEmpty()){
                        Toast.makeText(this@CompetidoresActivity, "No se encontraron competidores inscritos en esta prueba", Toast.LENGTH_SHORT).show()
                        return
                    }
                    resultados.forEach {
                    ...
                    Toast.makeText(this@CompetidoresActivity, "Resultados cargados correctamente", Toast.LENGTH_SHORT).show()
                } else {
                    Toast.makeText(this@CompetidoresActivity, "Error en la respuesta", Toast.LENGTH_SHORT).show()
                }
            }

            override fun onFailure(call: Call<List<Resultado>>, t: Throwable) {
                Toast.makeText(this@CompetidoresActivity, "Error en la conexión", Toast.LENGTH_SHORT).show()
